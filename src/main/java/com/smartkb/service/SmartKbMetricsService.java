...
public class SmartKbMetricsService {
    ...
    private final Counter rerankerRequestsTotal;
    private final Counter rerankerFallbackTotal;
    private final Timer rerankerDuration;
    private final Counter lowConfidenceRefusalTotal;
    ...
    public SmartKbMetricsService(MeterRegistry registry) {
        ...
        rerankerRequestsTotal = Counter.builder("smartkb.reranker.requests")
                .description("BGE reranker request count")
                .register(registry);

        rerankerFallbackTotal = Counter.builder("smartkb.reranker.fallback")
                .description("BGE reranker fallback count")
                .register(registry);

        rerankerDuration = Timer.builder("smartkb.reranker.duration")
                .description("BGE reranker request duration")
                .publishPercentileHistogram()
                .register(registry);

        lowConfidenceRefusalTotal = Counter.builder("smartkb.rag.refusal")
                .description("Low-confidence RAG refusal count")
                .register(registry);
    ...
    public void recordRerankerCall(long durationMs, boolean fallback) {
        rerankerRequestsTotal.increment();
        if (fallback) {
            rerankerFallbackTotal.increment();
        }
        if (durationMs > 0) {
            rerankerDuration.record(java.time.Duration.ofMillis(durationMs));
        }
    }
