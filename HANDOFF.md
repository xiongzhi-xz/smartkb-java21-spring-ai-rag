...
- Surefire and Failsafe now pass the Spring Boot-managed Mockito 5.17.0 JAR as an explicit Java agent and enable unnamed-module native access. Temurin 25.0.3 `mvn -B clean verify`, the self-contained integration profile, and an isolated real Milvus/OpenSearch `RetrievalBackendsSmokeIT` smoke all passed without Mockito self-attachment, dynamic-agent, or restricted-native-access warnings. The isolated `smartkb-java25-smoke` Compose project used separate host ports and was removed with its temporary volumes afterwards.
...
- Before the integration-profile split, `mvn -B -Dapi.version=1.40 -P integration-tests verify` passed `EnterpriseRagPersistenceIT` (2 tests) against PostgreSQL 16.14 with Flyway V1/V2/V3, then selected retrieval smoke ITs without their Milvus/OpenSearch services or `smartkb.degradation.scenario`. That external-smoke precondition was not a Java 25 failure; the profile now excludes those tests by default.
- Remaining Java 25 compatibility risk: Lombok 1.18.40 emits a terminal-deprecation warning for `sun.misc.Unsafe::objectFieldOffset` while Maven compiles. It does not fail Java 25 builds or tests; do not upgrade it solely to suppress this warning unless a later JDK validation actually fails.
