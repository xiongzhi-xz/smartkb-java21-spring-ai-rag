...
7. 点击“质量评测”，展示普通向量召回、Advanced RAG 和引用片段的命中率对比。
8. 停在引用定位或质量评测结果，说明核心价值是“答案可以回到原文 chunk，并且检索效果可评估”，不是纯聊天输出。
...
RAG 质量评测把“Advanced RAG 更好”从主观演示变成可比较指标：同一组中文问题分别看普通向量召回和 Advanced RAG 是否命中预期章节，同时统计引用片段是否覆盖期望关键词。第一版只评检索和引用，不评模型生成文本，结果更稳定。
