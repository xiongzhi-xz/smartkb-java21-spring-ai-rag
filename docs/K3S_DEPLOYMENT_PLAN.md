...
```powershell
curl.exe http://localhost:18080/api/rag/eval/report
```
...
- Sets the PostgreSQL, Redis, Ollama, and Chat API runtime configuration.
...
- Verified path:
  ...
  - verified `http://localhost:18080/api/rag/eval/report` returned successfully without requiring an LLM call.
