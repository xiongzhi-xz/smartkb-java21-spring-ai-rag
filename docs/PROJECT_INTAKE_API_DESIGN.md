...
1. 新增请求/响应 DTO 和 `ProjectIntakeController`。
2. 新增 `ProjectPathGuard`，实现路径存在性、目录校验、敏感路径跳过。
3. 新增 `ProjectFileReader`，按白名单读取文档和文件树。
4. 新增 `GitCommandReader`，读取 `status --short` 和 `log --oneline -5`。
5. 新增 `ProjectIntakeService`，汇总目标、阶段、已完成、未完成、风险和下一步。
6. 增加单元测试：路径非法、非 Git 仓库、敏感文件跳过、TicketRush fixture 摘要。
