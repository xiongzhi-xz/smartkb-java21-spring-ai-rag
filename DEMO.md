...
- 页面显示内置中文评测用例数量
- 对比普通向量召回和 Advanced RAG 是否召回预期 chunk
- 展示 Advanced Recall@K、Top1、MRR、引用覆盖和失败原因

设计说明：这一步回答“怎么证明 RAG 效果”的问题。评测只使用固定中文问题集，评估检索和引用片段，不评当前聊天历史，也不把 LLM 生成文本纳入评分，避免模型输出随机性影响质量指标。
