...
- [x] RAG 检索质量评测默认中文用例（预期 chunk、普通向量召回 vs Advanced RAG）
- [x] RAG 检索质量评测报告 API（Recall@K、Top1、MRR、引用覆盖、失败原因）
- [x] 已上传文档详情窗口可拖动调整宽度
...
- [x] 刷新页面后同一 conversationId 设计问题，LLM 能记住之前的对话
- [x] 重启 Spring Boot 后同一 conversationId 设计问题，LLM 能记住之前的对话
- [x] Advanced 模式同样共享 conversationId，设计问题时查询改写能基于历史
...
- [x] K3s demo manifest 一次性 K3d 运行态验证（Pod Running、PVC Bound、health `UP`、Eval report API）
- [x] K8s 清单命名收口（已验证 `k3s-demo.yaml` / 历史草案 `deployment-draft.yaml`）
- [x] K8s draft manifest 防误用守卫测试（警告、占位 Secret、README 部署入口）
...
- [x] AgentTask 任务列表摘要指标（总数、进行中、已记录、待介入）
...
- [x] Project Intake 优先解析 HANDOFF `Latest Snapshot`（Current goal/stage、Recently completed、Next step only）
- [x] Project Intake 前端接管报告增强（当前目标、当前阶段、已完成、未完成、工作区状态、下一步只做）
...
- [x] GitHub 展示首页 README 收口（RAG 主链路叙事、启动、演示、API、验证状态；工程工作台降级为可选实验并从首页隐藏）
- [x] GitHub 展示补充页（快速看点、浏览顺序、项目摘要和边界说明）
- [x] GitHub README 桌面横屏 RAG 主链路截图（上传、入库、文档详情、普通问答、多轮追问、Advanced RAG、引用片段定位、检索质量评测）
...
这些能力保留为“面向 Java 存量项目的 AI 接管与开发辅助”实验，不进入当前 5 分钟主演示，也不占用首页导航。主线仍然是 SmartKB 的 RAG 知识库闭环：上传、切片、检索、问答、追问、Advanced RAG 和引用片段定位。
...
Eval 技术总结见：`docs/EVAL_TECHNICAL_SUMMARY.md`。
