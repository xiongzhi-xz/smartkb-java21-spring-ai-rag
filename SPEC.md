...
- [x] conversation 模式提问后，`redis-cli keys smartkb:chat:*` 能看到会话 Key（Chat 调用因 token 无效返回 401，但 `MessageChatMemoryAdvisor` 已写入用户消息）
...
- [x] 点击"新会话"后，Redis 中对应 Key 被删除，下次提问是新会话（已用后端 DELETE `/api/chat/memory/{conversationId}` 验证）
